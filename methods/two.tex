...
a finite set of all possible PPSs that can be derived from it (the length of a
PPS in this study is subject to constraints $150 \leq length(PPS) \leq 1e4$):
...
interested in the topmost hit and its bit-score denoted $b_{A \rightarrow B}^i$. More
formally:
...
a $g$-``core''. Conversely, a $g_A$ assigned to $B$ is an $g$-``outsider''. As
